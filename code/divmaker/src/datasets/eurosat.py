...
class TripletEuroSAT(BaseEuroSAT):
    ...
    def __init__(
        ...
        self.triplets_mapping = self.create_triplets()
        
    def create_triplets(self, use_labels=False):
        triplets_mapping = {}
        if use_labels:
            labels = np.array([self.dataset.__getitem__(i)[1] for i in range(len(self.dataset))])
        for i in range(len(self.dataset)):
            if use_labels:
                ...
                triplets_mapping[i] = negative_index
            else:
                indexes = list(range(len(self.dataset)))
                indexes.remove(i)
                negative_index = random.choice(indexes)
                triplets_mapping[i] = negative_index
    ...
    def __getitem__(self, index):
        img_data, label = self.dataset.__getitem__(index)
        img2_data, _ = self.dataset.__getitem__(index)
        img3_data, _ = self.dataset.__getitem__(self.triplets_mapping[index])
        data = [index, img_data.float(), img2_data.float(), img3_data.float(), label, label, label]
        return tuple(data)
