...
model_name = 'TileNet_128_dims.ckpt'
...
for epoch in range(0, epochs):
        (avg_loss, avg_l_n, avg_l_d, avg_l_a) = train_triplet_epoch(
            TileNet, cuda, dataloader, optimizer, epoch+1, margin=margin, l2=l2,
            print_every=print_every, t0=t0)
        
        if save_models and epochs % 50:
            print("saving model")
            model_fn = os.path.join(model_dir, str(model_name, "epoch_", epoch, ".ckpt"))
            torch.save(TileNet.state_dict(), model_fn)
        
        avg_losses.append(avg_loss)
        avg_l_ns.append(avg_l_n)
        avg_l_ds.append(avg_l_d)
        avg_l_as.append(avg_l_a)
...
with open(results_path / str(model_name + ".pkl"), "wb") as f:
    avg = {"losses": avg_losses, "l_n": avg_l_ns, "l_d": avg_l_ds, "l_a": avg_l_as}
    pickle.dump(avg, f)